...
This quirk displays retained marker state associated with the gauge reading, such as a minimum, maximum, follower, tell-tale, or damped secondary marker position.

For the current GoDriveLog `bar` gauge, the behaviour applies to displayed fill or reveal extent only. It must not alter the input sensor value, configured ranges, exported values, or logs.
...
| Key | Meaning |
|---|---|
| `pointer_markers.max` | Tracks the highest/furthest final rendered bar fill or indicator position in the active min/max history mode. |
| `pointer_markers.min` | Tracks the lowest/least final rendered bar fill or indicator position in the active min/max history mode. |
| `pointer_markers.average` | Shows an old-style highly damped marker. It is not a mathematical average. |
...
- a **maximum marker** showing the highest reached bar position;
- a **minimum marker** showing the lowest reached bar position;
- a **damped follower marker** that trails the main bar movement, like a slow mechanical witness pointer or highly damped secondary indicator.

For bar gauges, these are marker ticks or overlays placed along the bar travel.

## Source of truth

Pointer markers observe the gauge's final rendered indicator position.

For the current GoDriveLog `bar` gauge, that means the final rendered fill, reveal, or indicator position after value mapping and any enabled display-only realism effects.

If no realism effects are enabled, that rendered indicator path is equivalent to the mapped source value. If realism effects are enabled, pointer markers follow the realistic rendered behaviour. For example, a bar max marker may capture visible overshoot if the live fill or reveal extent overshoots.
...
`min` and `max` markers show the extremes reached by the rendered bar indicator.

### Real-world analogue

This simulates tell-tale, witness, or peak-hold markers on physical instruments. These markers are useful where the operator cares about the highest or lowest reached reading.
...
The min and max markers clearly show the reached extremes without obscuring the live bar display.
...
`average` is an old-style highly damped marker. It follows the live bar indicator slowly and calmly.

It is not a mathematical average.

### Real-world analogue

This simulates a secondary damped follower pointer or marker with much more inertia than the live indicator. It gives the impression of an older mechanical instrument where a slow marker shows the general trend while the main indicator shows the current value.

In v3.6, this marker uses a fixed 10 second time constant.

### Good result

The average marker trails the live bar indicator smoothly and feels like a physical damped follower.
...
Pointer markers render above the live bar indicator and below overlay, glass, bezel, and frame layers.

Expected bar marker assets:

| Marker | Asset | Notes |
|---|---|---|
| Minimum | `marker_min.png` | Placed along the bar axis and respects the bar direction/origin. |
| Maximum | `marker_max.png` | Placed along the bar axis and respects the bar direction/origin. |
| Average / damped follower | `marker_average.png` | Placed along the bar axis and respects the bar direction/origin. |

## Good result

The markers feel like part of the physical instrument: visible, useful, quiet, and clearly tied to the live bar indicator.
